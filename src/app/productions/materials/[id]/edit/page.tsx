...
export default function EditMaterialPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      router.push('/productions/materials');
    } catch (error) {
      console.error('Error saving material:', error);
      setError(error instanceof Error ? error.message : 'Failed to save material');
    } finally {
      setSaving(false);
    }
  };
          ...
          <div className="px-6 py-4 border-b border-gray-200">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <h1 className="text-xl font-semibold text-gray-900">Edit Material</h1>
              </div>
              <button
                type="button"
                onClick={() => router.back()}
